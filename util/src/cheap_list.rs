...
pub trait EntBase<Type> : DerefMut<Target = Type> {
    fn get_next(&self) -> *const Link;
    fn set_prev(&mut self, ent: *const Link);
    fn set_next(&mut self, ent: *const Link);
    fn ref_elem(&self) -> &Type;
    fn ref_elem_mut(&mut self) -> &mut Type;
}

// ForwardEnt

pub struct ForwardEnt<Type> {
    next: *const Link,
    elem: Type,
}

impl<Type> ForwardEnt<Type> {
...
impl<Type: Clone> Clone for ForwardEnt<Type> {
    fn clone(&self) -> Self {
        Self {
            next: core::ptr::null(),
...
impl<Type: Copy> Copy for ForwardEnt<Type> {}

impl<Type> Deref for ForwardEnt<Type> {
    type Target = Type;
...
impl<Type> DerefMut for ForwardEnt<Type> {
    fn deref_mut(&mut self) -> &mut Self::Target {
        self.ref_elem_mut()
    }
}

impl<Type: PartialEq> PartialEq for ForwardEnt<Type> {
    fn eq(&self, other: &Self) -> bool {
        self.elem == other.elem
    }
}

impl<Type: fmt::Debug> fmt::Debug for ForwardEnt<Type> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        self.elem.fmt(f)
    }
}

impl<Type> EntBase<Type> for ForwardEnt<Type> {
    fn get_next(&self) -> *const Link {
        self.next
    }
...
impl<Type> Into<RawRefer<ForwardEnt<Type>>> for &mut ForwardEnt<Type> {
    fn into(self) -> RawRefer<ForwardEnt<Type>> {
        RawRefer {
            ptr: unsafe { NonNull::<ForwardEnt<Type>>::new_unchecked(self) }
        }
    }
}
...
impl<Type: Clone> Clone for Ent<Type> {
    fn clone(&self) -> Self {
        Self {
            next: core::ptr::null(),
            prev: core::ptr::null(),
...
impl<Type: Copy> Copy for Ent<Type> {}

impl<Type> Deref for Ent<Type> {
    type Target = Type;
...
impl<Type> DerefMut for Ent<Type> {
    fn deref_mut(&mut self) -> &mut Self::Target {
        self.ref_elem_mut()
    }
}
impl<Type: PartialEq> PartialEq for Ent<Type> {
    fn eq(&self, other: &Self) -> bool {
        self.elem == other.elem
    }
}

impl<Type: fmt::Debug> fmt::Debug for Ent<Type> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.write_fmt(format_args!("Ent {{ next: {:?}, prev: {:?}, ", self.next, self.prev));
        self.elem.fmt(f);
        f.write_fmt(format_args!("}}"));
        Ok(())
    }
}
impl<Type> EntBase<Type> for Ent<Type> {
    fn get_next(&self) -> *const Link {
        self.next
    }
    fn set_prev(&mut self, ent: *const Link) {
...
impl<Type> Into<RawRefer<Ent<Type>>> for &mut Ent<Type> {
    fn into(self) -> RawRefer<Ent<Type>> {
        RawRefer {
            ptr: unsafe { NonNull::<Ent<Type>>::new_unchecked(self) }
        }
    }
}

impl<Type> Ent<Type> {
    fn get_prev(&self) -> *const Link {
        self.prev
    }
}
...
pub struct ListImpl<Type, Ref, End, Entry> 
    where
        Ref: Refer<Entry>,
        End: EndBase,
        Entry: EntBase<Type> {
    ...
    _phantom3: PhantomData<Entry>,
}

impl<Type, Ref, End, Entry> Clone for ListImpl<Type, Ref, End, Entry>
    where
        Ref: Refer<Entry>,
        End: EndBase,
        Entry: EntBase<Type> {
...
impl<'e, Type, Ref, End, Entry> ListImpl<Type, Ref, End, Entry>
    where
        Ref: Refer<Entry>,
        End: EndBase,
    ...
    fn _ref_ent(ent: *const Link) -> &'e mut Entry {
        unsafe { &mut *(ent as *mut Entry) }
    }

    fn _ref_ent_or_none(ent: *const Link) -> Option<&'e mut Entry> {
        unsafe { (ent as *mut Entry).as_mut() }
    }

    pub fn get_front(&self) -> Option<&'e mut Entry> {
        let ret_raw = self.end.get_head() as *mut Entry;
        unsafe { ret_raw.as_mut() }
    }

    pub fn get_next(&self, base: &Entry) -> Option<&'e mut Entry> {
        let ret_raw = base.get_next() as *mut Entry;
        unsafe { ret_raw.as_mut() }
    }

    pub fn push_front(&mut self, mut ent: Ref) {
        let old_head = self.end.get_head();
        (*ent).set_prev(core::ptr::null());
        (*ent).set_next(old_head);
        let new_head = ent.into_ptr() as *const Link;

        self.end.set_head(new_head);
        if !old_head.is_null() {
            Self::_ref_ent(old_head).set_prev(new_head);
        } else {
            self.end.set_tail(new_head);
        }
    }
    ...
    pub fn pop_front(&mut self) -> Option<Ref> {
        let old_head = self.end.get_head();
        if !old_head.is_null() {
            let new_head = Self::_ref_ent(old_head).get_next();
            self.end.set_head(new_head);
            if !new_head.is_null() {
                Self::_ref_ent(new_head).set_prev(core::ptr::null());
            } else {
...
impl<'e, Type, Ref, Entry> ListImpl<Type, Ref, DualEnd, Entry>
    ...
    pub fn get_back(&self) -> Option<&'e mut Entry> {
        let ret_raw = self.end.get_tail() as *mut Entry;
        unsafe { ret_raw.as_mut() }
    }

    pub fn push_back(&mut self, mut ent: Ref) {
        let old_tail = self.end.get_tail();
        ...
        if !old_tail.is_null() {
            Self::_ref_ent(old_tail).set_next(new_tail);
        } else {
...
impl<'e, Type, Ref> ListImpl<Type, Ref, DualEnd, Ent<Type>>
    ...
    pub fn pop_back(&mut self) -> Option<Ref> {
        let old_tail = self.end.get_tail();
        if !old_tail.is_null() {
            let new_tail = Self::_ref_ent(old_tail).get_prev();
            self.end.set_tail(new_tail);
            if !new_tail.is_null() {
                Self::_ref_ent(new_tail).set_next(core::ptr::null());
            } else {
...
pub type List<Type, Ref> =
    ListImpl<Type, Ref, DualEnd, Ent<Type>>;
pub type SingleList<Type, Ref> =
    ListImpl<Type, Ref, SingleEnd, Ent<Type>>;
pub type ForwardList<Type, Ref> =
    ListImpl<Type, Ref, DualEnd, ForwardEnt<Type>>;
pub type SingleForwardList<Type, Ref> =
    ListImpl<Type, Ref, SingleEnd, ForwardEnt<Type>>;
...
impl<'e, Type, Ref, End, Entry> Iterator for ListIter<'e, Type, Ref, End, Entry>
    ...
    fn next(&mut self) -> Option<&'e mut Type> {
        if self.next.is_null() {
            ...
            let ret_ent =
                ListImpl::<Type, Ref, End, Entry>::_ref_ent(self.next);
...
impl<'e, Type, Ref> DoubleEndedIterator
    ...
    fn next_back(&mut self) -> Option<&'e mut Type> {
        if self.next_back.is_null() {
            ...
            let ret_ent = ListImpl::<Type, Ref, DualEnd, Ent<Type>>::_ref_ent(
                self.next_back);
...
mod test {
    ...
    impl AsRef<Ent<Data>> for Ent<Data> {
        fn as_ref(&self) -> &Ent<Data> { &self }
    }
    impl AsRef<ForwardEnt<Data>> for ForwardEnt<Data> {
        fn as_ref(&self) -> &ForwardEnt<Data> { &self }
    }
    impl<T> AsRef<T> for RawRefer<T> {
